...
export const StaticSide: React.FC = () => {
  // No observers here. Everything is rendered statically.

  return (
    <div className="w-full min-h-full bg-stone-50 text-gray-800 font-sans">
      
      {/* Hero Section - Same design, no animations */}
      <div className="relative h-[85vh] w-full overflow-hidden flex items-center justify-center">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/50 to-stone-900/40"></div>
        
        <div className="relative z-10 text-center text-white p-8">
            <p className="font-sans text-xs md:text-sm tracking-[0.3em] uppercase text-gold-200 mb-4">
                Maison d'Élégance
            </p>
      ...
      {/* History Section - Same layout, no scroll reveal */}
      <div className="py-32 px-10 max-w-4xl mx-auto">
        ...
        <div className="space-y-24 relative before:absolute before:inset-0 before:ml-[50%] before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gold-300 before:to-transparent before:hidden md:before:block">
            {HISTORY_MILESTONES.map((milestone, i) => (
                <HistoryItemStatic key={i} milestone={milestone} index={i} />
            ))}
        </div>
      </div>

      {/* Process Section - ZigZag Layout, no scroll reveal */}
      <div className="bg-stone-900 text-stone-200 py-32 overflow-hidden">
         ...
         <div className="max-w-6xl mx-auto px-6 space-y-32">
            {PROCESS_STEPS.map((step, index) => (
                <ProcessItemStatic key={step.id} step={step} index={index} />
            ))}
         </div>
      </div>
      ...
      <footer className="bg-stone-900 text-stone-200 py-24 px-6 relative overflow-hidden">
         ...
         <div className="max-w-4xl mx-auto text-center relative z-10">
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-8 md:gap-16 mb-16 text-sm tracking-widest uppercase text-gold-400">
                <span className="border-b border-transparent hover:border-gold-400 cursor-pointer">atelier@maisondelegance.com</span>
                <span className="border-b border-transparent hover:border-gold-400 cursor-pointer">+39 055 123 4567</span>
                <span className="border-b border-transparent hover:border-gold-400 cursor-pointer">Via Roma 12, Firenze</span>
            </div>

            <button className="text-xs uppercase tracking-[0.2em] border border-gold-400/30 px-10 py-4 hover:bg-gold-900/20 hover:border-gold-400 transition-none">
                Agendar Visita
            </button>
         </div>
...
// --- Sub-components (Static Versions) ---

const HistoryItemStatic: React.FC<{ milestone: any, index: number }> = ({ milestone, index }) => {
...
const ProcessItemStatic: React.FC<{ step: any, index: number }> = ({ step, index }) => {
    const isEven = index % 2 === 0;
